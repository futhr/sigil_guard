defmodule SigilGuard.MCP.GatewayTest do
  ...
  describe "issue_signed_confirmation_token/5" do
    test "fails closed because legacy envelope verification is removed" do
  ...
  describe "verify_request_envelope/2" do
    test "rejects legacy _sigil metadata because envelope verification is removed" do
      ...
      assert Gateway.verify_request_envelope(request, public_keys: public_keys()) ==
    ...
    test "does not let atom identity fallback mask explicit invalid identity" do
      envelope =
        legacy_envelope("did:sigil:agent")
  ...
  describe "guard_signed_request/3" do
    test "blocks legacy signed requests before tool execution" do
      ...
      assert decision.verdict == :blocked
      assert decision.action == :block
      assert decision.reason =~ "legacy_envelope_removed"
      assert decision.audit_metadata.envelope_reason == :legacy_envelope_removed
      assert decision.audit_metadata.tool == "read_file"
    end
    ...
    test "emits MCP telemetry for removed legacy signed requests" do
      ...
      assert metadata.envelope_reason == :legacy_envelope_removed
  ...
  describe "guard_signed_confirmed_request/3" do
    ...
    test "caller-supplied actor is not overwritten by removed legacy envelope claims" do
      ...
      assert decision.verdict == :blocked
      assert decision.audit_metadata.identity == "spoofed"
      assert decision.audit_metadata.actor == "spoofed"
      assert decision.audit_metadata.envelope_reason == :legacy_envelope_removed
    end
    ...
    test "emits MCP telemetry for removed legacy signed confirmations" do
      ...
      assert metadata.envelope_status == :invalid
      assert metadata.envelope_reason == :legacy_envelope_removed
      refute Map.has_key?(metadata, :confirmation_status)
  ...
  describe "response_for_decision/3" do
    ...
    test "emits manifest drift registry errors" do
      decision =
        registry_decision(:manifest_digest_mismatch,
          tool: "repo_file_write",
          mcp_server: "repo-mcp",
          expected_manifest_digest: "expected",
          received_manifest_digest: "received",
          drifted_fields: [:input_schema, :annotations]
        )
      ...
      assert data["expected_manifest_digest"] == "expected"
      assert data["received_manifest_digest"] == "received"
      assert data["drifted_fields"] == ["input_schema", "annotations"]
    end

    test "emits unknown and expired manifest registry errors" do
      unknown = Gateway.response_for_decision(registry_decision(:unknown_manifest), "unknown")

      expired =
        Gateway.response_for_decision(
          registry_decision(:manifest_expired, expires_at: "2026-06-30T12:00:00Z"),
          "expired"
        )
    ...
    test "emits invalid attestation registry errors" do
    ...
    test "emits sandbox-required registry errors" do
      decision =
        registry_decision(:sandbox_required,
          tool: "repo_file_write",
          required_isolation: "vm",
          received_isolation: "container",
      ...
      assert data["received_isolation"] == "container"
      assert data["sandbox_id_present"] == true
    end
  end
  ...
  defp registry_decision(deny_reason, metadata \\ []) do
    metadata = Map.merge(Map.new(metadata), %{deny_reason: deny_reason})

    %Decision{
      verdict: :blocked,
      action: :block,
      reason: Atom.to_string(deny_reason),
      phase: :tool_request,
      risk_level: :high,
      trust_level: :medium,
